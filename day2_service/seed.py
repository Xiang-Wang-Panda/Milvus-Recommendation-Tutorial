...
每个请求带 X-Tenant-Id（TENANT 环境变量，默认 default）。
灌完自动跑一次 compact——BM25 的 idf 按 segment 统计，不合并的话刚灌的批次排序不可比。

Day 2 默认灌 products_loo.csv / users_loo.csv（数据用全的那份），
要用 Day 0/1 那份简化数据就 PRODUCTS_CSV=products.csv USERS_CSV=users.csv。
"""
import csv
import os
import sys
...
if __name__ == "__main__":
    ...
    show("搜索 hybrid: a movie about time loops（BM25 + 语义两路 RRF 融合）",
         post("/search", {"q": "a movie about time loops and paradoxes",
                          "top_k": 5, "mode": "hybrid"}),
         lambda h: f"{h['score']:6.4f}  {h['text']}")

    uid = users()[0]["user_id"]
    for mode in ("content", "cf", "multi"):
        show(f"推荐 {uid} mode={mode}（已排除历史）",
             post("/recommend", {"user_id": uid, "top_k": 5, "mode": mode}),
             lambda h: f"{h['score']:6.3f}  {h['text']}")
    show(f"推荐 {uid} mode=multi + 按 category 分组去重",
         post("/recommend", {"user_id": uid, "top_k": 5, "mode": "multi",
                             "group_by": "category"}),
         lambda h: f"{h['score']:6.4f}  {h['text']}")
    show(f"相似用户 {uid}",
