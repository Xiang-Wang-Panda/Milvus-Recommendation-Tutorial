"""Day 1 服务层。相对 Day 0 只多了两件事：每个请求带租户、每个请求出埋点。"""
import os
import time
...
app = FastAPI(title="Day 1 搜索 / 推荐（集群 · 多租户）")
store.init()
...
def recommend(r: Rec, t: T):
    if r.user_id:
        ...
        return store.recommend(t, u["dense"], r.top_k, u["history"].split("|"), r.filter)
    vecs = store.item_vectors(t, r.item_ids or [])
    if not vecs:
        raise HTTPException(400, "need user_id or known item_ids")
    return store.recommend(t, store.mean(vecs), r.top_k, r.item_ids, r.filter)
